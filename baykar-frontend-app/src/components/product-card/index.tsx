...
function Card(props: CardProps) {
  ...
  return (
    <div
      className="bg-main-color-3 z-10 grid max-w-[394px] grid-cols-1 rounded-[10px] bg-blue text-white transition hover:scale-105"
      style={{
        boxShadow: "0px 0px 15px 0px #FFFFFF12, 0px 25px 50px -12px #FFFFFF40",
      }}
    >
      <div>
        <img className="rounded-t-[10px]" src={props.image} alt="rectangle" />
      </div>
      <div className="space-y-4 px-8 pt-8">
        <h3 className="text-2xl font-bold leading-[26.4px]">{props.title}</h3>
      ...
      <div className="p-8">
        <Button
          ...
          className={clsx(
            "flex w-full justify-center text-xl  font-medium leading-6 tracking-[0.5px]",
            "rounded-lg px-10 py-4 text-white ring-[2px] ring-white hover:bg-white hover:text-blue",
          )}
          icon={cartIcon}
        />
      </div>
...
export default function ProductCard() {
  return (
    <div className="flex flex-col items-center justify-center gap-8 md:justify-between lg:flex-row lg:gap-12">
